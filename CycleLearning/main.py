...
from cadence_controller.cadence_controller import CadenceController
from datastructures import biased_reservoir_sampling
from params import seqlen

cadence_controller = CadenceController(
    RandomForestRegressor(max_depth=4, n_estimators=20),
    data_structure=biased_reservoir_sampling.BiasedReservoirSampling(seqlen * 20),
    ptype="none", seqlen=seqlen, verbose=False, normalize=False,
    stochastic=True)
# enkel een fietsersmodel op basis van gemiddeld koppel/helling zijn uitgewerkt
cycle1 = Cycle(cycle_model=(lambda cm_tdc, cm_speed, cm_slope: 7.5 * cm_tdc))
cycle2 = Cycle(cycle_model=(lambda cm_tdc, cm_speed, cm_slope: 300 * cm_slope + 60))


def fiets(bicycle, cc, timesteps, update=True):
    '''
    This function simulates a bicycle tour of #timestep iterations.
    :param bicycle: the cycle used
    :param cc: the cadence controller
    :param timesteps: #iterations
    :param update: whether to update the cadence controller or not
    '''
    for h in range(1, timesteps):
        ...
        cc.update_error(h, predicted_rpm, fcc_rpm)
    cc.stats()
...
#cycle1.save()
# fiets(cycle2,cadence_controller,4000)
